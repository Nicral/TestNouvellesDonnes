function retrouveIndex() {
    ...
    if (idx != -1) {
        ...
        else { document.querySelector('#testUn').innerText = `Le nombre ${nombreTest} est en position ${indices} dans le tableau.` }
    }
    else { document.querySelector('#testUn').innerText = `le nombre n'est pas présent dans le tableau` }
}
...
function comparaisonParam() {
    ...
    if (paramUn === paramDeux && paramUn !== "" && paramDeux !== "") {
        ...
        document.querySelector('#testDeuxComparaison').innerText = `L'un ou les deux paramètres sont vide`;
    }
    else {document.querySelector('#testDeuxComparaison').innerText = `0`;}
}